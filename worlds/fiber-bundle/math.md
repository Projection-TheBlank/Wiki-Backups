...
我们采用$N_0 = N(0) = 1$ 时的表达式: $N(t) = e^{-\lambda t}$。假设在 $t = 0$ 时有很多的粒子。经过一段时间，设此时 $t = t_1$，还剩下的粒子的数量是初始的 $N(t_1)$ 倍，在 $0$ 到 $t_1$ 的这段时间，衰变了总量的 $1 - N(t_1)$ 倍的粒子，这些粒子的寿命在 $0$ 到 $t_1$ 之间。又过了一段时间，设此时 $t = t_2$，还剩下的粒子的数量是初始的 $N(t_2)$ 倍，衰变了总量的 $N(t_1) - N(t_2)$ 倍的粒子，这些粒子的寿命在 $t_1$ 到 $t_2$ 之间。